export const oncGnb = () => { //함수를 선언만 함 
    ...
    for(let i=0;i<main.length; i++){
        ...
        main[i].addEventListener('focus',function(){
            //모든 메인 메뉴 비활성 후 + 모든 하위 메뉴 비활성화 + 모든 하위 메뉴 초점 못받게 처리 
            ...
            //이벤트 받은 메인 메뉴의 동생만 활성 
            this.nextElementSibling.classList.add('active');

            //이벤트 받은 메인 메뉴의 동생의 a태그만 초점받게 처리
            const activeLink = this.nextElementSibling.getElementsByTagName('a');
            for(let j=0;j<activeLink.length;j++) {
                activeLink[j].setAttribute('tabindex', '0'); //하위메뉴의 개수는 모두 다르기때문에 activeLink로 해줘야함
            }
        });
    ...
    main[0].addEventListener('keydown', function(e){
        ...
        if(e.keyCode == 9){ //tap키 눌렀을 때
            if(e.shiftKey) { //shift키도 눌렀다면
