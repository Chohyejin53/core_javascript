...
//2. 버튼 클릭이벤트 - 폼 데이터를 전송하는 기본이벤트를 갖고 있음 
btn.addEventListener('click',function(e){
    //기본이벤트 제거 - 데이터 전송 막기
    e.preventDefault();

    //입력상자 6개가 모두 입력이 되었다면 데이터 전송 
    //그렇지않으면 경고창을 띄우시오.
    ...
    if(v01 !='' && v02 !='' && v03 !='' && v04 !='' && v05 !='' && v06 !=''){ //모두 비어있지 않다면    
...
for(let i=0;i<input.length;i++){
    input[i].addEventListener('blur',function(){
        //초점을 벗어난 input의 value값을 담는 변수 
        const v = this.value;

        //아이디입력상자 = 0
        //정규표현식  - 영어 소문자 + 숫자 + 밑줄과 가운데줄, 5~20글자
        ...
        //비밀번호 == 1 
        //정규표현식 : 영어대소문자숫자특수문자쓰고, 8~16글자 입력
        //() : 캡쳐할 그룹 
        //?=.* : 임의의 글자 하나라도 일치하면됨
        //\d : 숫자에 일치
        //\W : 63개의 문자가 아닌 것과 일치 (특수문자 의미 / 반드시 대문자 W 사용)
        ...
        //비밀번호 확인 == 2 
        //기존 비밀번호를 담는 변수 
        const passValue = input[1].value; //1이어야함!
        ...
        //이메일 == 4 
        //정규표현식 : 이메일 양식 
        //+ : 1회 이상 연속적으로 반복되는 문자에 가능한 많이 일치 (빈드시한번은써서 일치해야함을 의미)
        const emailReg = /^[a-z0-9_+.-]+@([a-z0-9]+\.)+[a-z0-9]{2,4}$/;

        if ( i ==4 && !emailReg.test(v) && v  != '' ) {
            this.nextElementSibling.style.display = 'block';
            this.nextElementSibling.textContent = '이메일 주소를 다시 확인해주세요.';
